    ...
    az_span message_span)
{
  ...
  az_span property = AZ_SPAN_LITERAL_FROM_STR("Product1Name");

  bool property_found = false;

  // Parse message_span.
  az_json_reader jr;
  EXIT_IF_AZ_FAILED(az_json_reader_init(&jr, message_span, NULL), RESULT_ERROR, "Failed to get the commands response topic.");
  EXIT_IF_AZ_FAILED(az_json_reader_next_token(&jr), RESULT_ERROR, "Failed to get the commands response topic.");

  az_span token = AZ_SPAN_EMPTY;
  az_json_token_copy_into_span(&jr.token, token);

  LogInfo("Token (%.*s).", az_span_size(token), az_span_ptr(token));
...
int azure_iot_mqtt_client_message_received(azure_iot_t *azure_iot, mqtt_message_t *mqtt_message)
{
  ...
  if (azure_iot->state == azure_iot_state_ready)
  {
    ...
    if (az_result_succeeded(azrc))
    {
      ...
      case AZ_IOT_HUB_CLIENT_PROPERTIES_MESSAGE_TYPE_GET_RESPONSE:
        // PROP GET FUNCTION
        LogInfo("PROPERTY GET REQUEST");
        LogInfo("Parsing topic (%.*s).", az_span_size(mqtt_message->topic), az_span_ptr(mqtt_message->topic));


     
        azure_iot->config->on_properties_update_completed(999, property_message.status, mqtt_message->payload);
  ...
  //       azrc = az_iot_hub_client_twin_parse_received_topic(&azure_iot->iot_hub_client, mqtt_message->topic, &twin_response);
  //       if (az_result_failed(azrc))
  //       {
  //         LogInfo("Failed");
  //       }
  //       // parse_device_twin_message(mqtt_message->topic,  az_span_size(topic_len), mqtt_message->payload, &twin_response);

  //       switch (twin_response.response_type)
  //       {
  //       case AZ_IOT_HUB_CLIENT_TWIN_RESPONSE_TYPE_GET:
  //       LogInfo("Parsing topic (%.*s).", az_span_size(mqtt_message->payload), az_span_ptr(mqtt_message->payload));
  //         LogInfo("Message Type: GET");
  //         parse_desired_device_count_property(mqtt_message->payload);
  //         break;

  //       case AZ_IOT_HUB_CLIENT_TWIN_RESPONSE_TYPE_REPORTED_PROPERTIES:
  //         LogInfo("Message Type: Reported Properties");
  //         break;
  ...
  //         LogInfo("Message Type: Request Error");
  //         break;
  //       }
        result = RESULT_OK;
        break;
      ...
      case AZ_IOT_HUB_CLIENT_PROPERTIES_MESSAGE_TYPE_ACKNOWLEDGEMENT:
        ...
        if (azure_iot->config->on_properties_update_completed != NULL)
        {
          ...
          {
            azure_iot->config->on_properties_update_completed(request_id, property_message.status, AZ_SPAN_EMPTY);
            //  int packet_id = azure_iot->config->mqtt_client_interface.mqtt_client_publish(azure_iot->mqtt_client_handle, mqtt_message);
            LogInfo("Parsing properties update request id (%.*s).", az_span_size(property_message.request_id), az_span_ptr(property_message.request_id));
          }
        }
        break;
