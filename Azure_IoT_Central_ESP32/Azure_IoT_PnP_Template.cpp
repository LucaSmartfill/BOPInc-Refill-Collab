...
/* --- Defines --- */
// Change this to edit default template
#define AZURE_PNP_MODEL_ID "dtmi:smartfillDev:UnlileverDispenser_45k;1"
...
/* --- Definitions of telelmenetry props --- */

// Mass from load cells
#define TELEMETRY_PROP_NAME_Dispenser1Mass                    "Product1Mass"
#define TELEMETRY_PROP_NAME_Dispenser2Mass                    "Product2Mass"
#define TELEMETRY_PROP_NAME_Dispenser3Mass                    "Product3Mass"
#define TELEMETRY_PROP_NAME_Dispenser4Mass                    "Product4Mass"

#define TELEMETRY_PROP_NAME_EVENT                            "Event"
#define TELEMETRY_PROP_NAME_PRODUCT                          "Product"
#define TELEMETRY_PROP_NAME_PRICE                             "Price"
#define TELEMETRY_PROP_NAME_DATE                              "Date"

#define TELEMETRY_PROP_NAME_ENTERED_AMOUNT                   "EnteredAmount"
#define TELEMETRY_PROP_NAME_DISPENSED_AMOUNT                 "DispensedAmount"
...
static az_span COMMAND_NAME_TAKE_STOCK = AZ_SPAN_FROM_STR("TakeStock");

#define COMMAND_RESPONSE_CODE_ACCEPTED                 202
#define COMMAND_RESPONSE_CODE_REJECTED                 404

#define WRITABLE_PROPERTY_PRODUCT1_NAME         "Product1Name"
#define WRITABLE_PROPERTY_PRODUCT1_TIME         "Product1Time"
#define WRITABLE_PROPERTY_PRODUCT1_PRICE         "Product1Price"

#define WRITABLE_PROPERTY_PRODUCT2_NAME         "Product2Name"
#define WRITABLE_PROPERTY_PRODUCT2_TIME         "Product2Time"
#define WRITABLE_PROPERTY_PRODUCT2_PRICE        "Product2Price"

#define WRITABLE_PROPERTY_PRODUCT3_NAME         "Product3Name"
#define WRITABLE_PROPERTY_PRODUCT3_TIME         "Product3Time"
#define WRITABLE_PROPERTY_PRODUCT3_PRICE        "Product3Price"

#define WRITABLE_PROPERTY_PRODUCT4_NAME         "Product4Name"
#define WRITABLE_PROPERTY_PRODUCT4_TIME         "Product4Time"
#define WRITABLE_PROPERTY_PRODUCT4_PRICE        "Product4Price"

#define WRITABLE_PROPERTY_PRODUCT2         "Product2"
#define WRITABLE_PROPERTY_PRODUCT3         "Product3"
#define WRITABLE_PROPERTY_PRODUCT4         "Product4"
...
static size_t telemetry_frequency_in_seconds = 1; // With default frequency of once in 5 minute 0 seconds.
static time_t last_telemetry_send_time = INDEFINITE_TIME;

static bool led1_on = false;
static bool led2_on = false;
// static string product ='test';
...
int azure_pnp_handle_command_request(azure_iot_t* azure_iot, command_request_t command)
{
  ...
  else if (az_span_is_content_equal(command.command_name, COMMAND_NAME_TAKE_STOCK))
  {
    response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
  }
  else
...
static int generate_telemetry_start_dispense_payload(uint8_t* payload_buffer, size_t payload_buffer_size, size_t* payload_buffer_length, char* event,char* product)
{
  ...
  az_span json_span;
  az_span product_span = az_span_create_from_str(product);
  az_span event_span = az_span_create_from_str(event);
  
  // Init Json writer and Json root
  ...
  // Start to append properties
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_EVENT));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 1 property name to telemetry payload.");

  rc = az_json_writer_append_string(&jw, event_span);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 1 property value to telemetry payload. ");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_PRODUCT));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property name to telemetry payload.");

  rc = az_json_writer_append_string(&jw, product_span);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property value to telemetry payload. ");
...
static int generate_telemetry_amount_chosen_dispense_payload(uint8_t* payload_buffer, size_t payload_buffer_size, size_t* payload_buffer_length, char* event,char* product, int eneteredAmount)
{
  ...
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property value to telemetry payload. ");
    ...
    rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_ENTERED_AMOUNT));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, eneteredAmount);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property value to telemetry payload. ");
...
static int generate_telemetry_dispense_payload(uint8_t* payload_buffer, size_t payload_buffer_size, size_t* payload_buffer_length, char* event,char* product, int eneteredAmount, int dispensedAmount, int price, char* date)
{
  // WORKING HERE
  az_json_writer jw;
  az_result rc;
  ...
  az_span event_span = az_span_create_from_str(event);
 az_span date_span = az_span_create_from_str(date);
  ...
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property value to telemetry payload. ");
    ...
    rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_DATE));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property name to telemetry payload.");
  rc = az_json_writer_append_string(&jw, date_span);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property value to telemetry payload. ");

if (eneteredAmount!=999){
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_ENTERED_AMOUNT));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, eneteredAmount);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property value to telemetry payload. ");
}
if (dispensedAmount!=999){
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_DISPENSED_AMOUNT));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, dispensedAmount);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property value to telemetry payload. ");
}
if (price!=999){
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_PRICE));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, price);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property value to telemetry payload. ");
}
// Close Json Payload
  rc = az_json_writer_append_end_object(&jw);
...
// This functions generates the telementray payload for the mass readings from each dispenser
static int generate_telemetry_payload(uint8_t* payload_buffer, size_t payload_buffer_size, size_t* payload_buffer_length, char* event, float Dispenser1Mass,float Dispenser2Mass,float Dispenser3Mass,float Dispenser4Mass )
{
  az_json_writer jw;
  az_result rc;
  az_span payload_buffer_span = az_span_create(payload_buffer, payload_buffer_size);
  ...
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_Dispenser1Mass));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 1 property name to telemetry payload.");

  rc = az_json_writer_append_double(&jw, Dispenser1Mass, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 1 property value to telemetry payload. ");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_Dispenser2Mass));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, Dispenser2Mass,DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 2 property value to telemetry payload. ");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_Dispenser3Mass));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 3 property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, Dispenser3Mass,DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 3 property value to telemetry payload. ");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_Dispenser4Mass));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 4 property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, Dispenser4Mass, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding dispenser 4 property value to telemetry payload. ");
  ...
  size_t* response_length)
{
  ...
  az_span res = az_span_create_from_str(product);
  azrc = az_json_writer_append_string(&jw, res);
  ...
  uint8_t* buffer, size_t buffer_size, size_t* response_length)
{
      ...
      AZ_IOT_HUB_CLIENT_PROPERTY_WRITABLE, &component_name)))
  {
    ...
    if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT1_NAME)))
    {
      azrc = az_json_reader_next_token(&jr);
      EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties next token.");
      char productName;
      azrc = az_json_token_get_string(&jr.token, &productName, 50, NULL);
      az_span productSpan = az_span_create_from_str(&productName);
      az_span productNameSpan = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT1_NAME);
      result = generate_properties_update_response(azure_iot, productNameSpan, &productName, version, buffer, buffer_size, response_length);
      EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "generate_properties_update_response failed.");

    }

       if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT1_PRICE)))
    {
     azrc = az_json_reader_next_token(&jr);
      EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties next token.");
      char productName;
      azrc = az_json_token_get_string(&jr.token, &productName, 50, NULL);
      az_span productSpan = az_span_create_from_str(&productName);
      az_span productNameSpan = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT1_PRICE);
      result = generate_properties_update_response(azure_iot, productNameSpan, &productName, version, buffer, buffer_size, response_length);
      EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "generate_properties_update_response failed.");

    }

    if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT1_TIME)))
    {
     azrc = az_json_reader_next_token(&jr);
      EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties next token.");
      char productTime;
      azrc = az_json_token_get_string(&jr.token, &productTime, 100, NULL);
      az_span productSpan = az_span_create_from_str(&productTime);
      // LogInfo("%s",productTime);
      az_span productNameSpan = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT1_TIME);
      result = generate_properties_update_response(azure_iot, productNameSpan, &productTime, version, buffer, buffer_size, response_length);
      EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "generate_properties_update_response failed.");

    }

        if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT2_NAME)))
    {
      azrc = az_json_reader_next_token(&jr);
      EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties next token.");
      char productName;
      azrc = az_json_token_get_string(&jr.token, &productName, 50, NULL);
      az_span productSpan = az_span_create_from_str(&productName);
      az_span productNameSpan = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT2_NAME);
      result = generate_properties_update_response(azure_iot, productNameSpan, &productName, version, buffer, buffer_size, response_length);
      EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "generate_properties_update_response failed.");

    }
   if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT2_PRICE)))
    {
     azrc = az_json_reader_next_token(&jr);
      EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties next token.");
      char productName;
      azrc = az_json_token_get_string(&jr.token, &productName, 50, NULL);
      az_span productSpan = az_span_create_from_str(&productName);
      az_span productNameSpan = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT2_PRICE);
      result = generate_properties_update_response(azure_iot, productNameSpan, &productName, version, buffer, buffer_size, response_length);
      EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "generate_properties_update_response failed.");

    }

         if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT2_TIME)))
    {
     azrc = az_json_reader_next_token(&jr);
      EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties next token.");
      char productName;
      azrc = az_json_token_get_string(&jr.token, &productName, 50, NULL);
      az_span productSpan = az_span_create_from_str(&productName);
      az_span productNameSpan = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT2_TIME);
      result = generate_properties_update_response(azure_iot, productNameSpan, &productName, version, buffer, buffer_size, response_length);
      EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "generate_properties_update_response failed.");

    }
        if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT3_NAME)))
    {
      azrc = az_json_reader_next_token(&jr);
      EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties next token.");
      char productName;
      azrc = az_json_token_get_string(&jr.token, &productName, 50, NULL);
      az_span productSpan = az_span_create_from_str(&productName);
      az_span productNameSpan = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT3_NAME);
      result = generate_properties_update_response(azure_iot, productNameSpan, &productName, version, buffer, buffer_size, response_length);
      EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "generate_properties_update_response failed.");

    }
   if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT3_PRICE)))
    {
     azrc = az_json_reader_next_token(&jr);
      EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties next token.");
      char productName;
      azrc = az_json_token_get_string(&jr.token, &productName, 50, NULL);
      az_span productSpan = az_span_create_from_str(&productName);
      az_span productNameSpan = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT3_PRICE);
      result = generate_properties_update_response(azure_iot, productNameSpan, &productName, version, buffer, buffer_size, response_length);
      EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "generate_properties_update_response failed.");

    }

         if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT3_TIME)))
    {
     azrc = az_json_reader_next_token(&jr);
      EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties next token.");
      char productName;
      azrc = az_json_token_get_string(&jr.token, &productName, 50, NULL);
      az_span productSpan = az_span_create_from_str(&productName);
      az_span productNameSpan = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT3_TIME);
      result = generate_properties_update_response(azure_iot, productNameSpan, &productName, version, buffer, buffer_size, response_length);
      EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "generate_properties_update_response failed.");

    }
        if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT4_NAME)))
    {
      azrc = az_json_reader_next_token(&jr);
      EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties next token.");
      char productName;
      azrc = az_json_token_get_string(&jr.token, &productName, 50, NULL);
      az_span productSpan = az_span_create_from_str(&productName);
      az_span productNameSpan = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT4_NAME);
      result = generate_properties_update_response(azure_iot, productNameSpan, &productName, version, buffer, buffer_size, response_length);
      EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "generate_properties_update_response failed.");

    }
   if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT4_PRICE)))
    {
     azrc = az_json_reader_next_token(&jr);
      EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties next token.");
      char productName;
      azrc = az_json_token_get_string(&jr.token, &productName, 50, NULL);
      az_span productSpan = az_span_create_from_str(&productName);
      az_span productNameSpan = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT4_PRICE);
      result = generate_properties_update_response(azure_iot, productNameSpan, &productName, version, buffer, buffer_size, response_length);
      EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "generate_properties_update_response failed.");

    }

         if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT4_TIME)))
    {
     azrc = az_json_reader_next_token(&jr);
      EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties next token.");
      char productName;
      azrc = az_json_token_get_string(&jr.token, &productName, 50, NULL);
      az_span productSpan = az_span_create_from_str(&productName);
      az_span productNameSpan = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT4_TIME);
      result = generate_properties_update_response(azure_iot, productNameSpan, &productName, version, buffer, buffer_size, response_length);
      EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "generate_properties_update_response failed.");

    }
    // if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT2)))
    // {
    //   azrc = az_json_reader_next_token(&jr);
    //   EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties next token.");
    //   char product;
    //   azrc = az_json_token_get_string(&jr.token, &product, 9999, NULL);
    //   az_span productSpan = az_span_create_from_str(&product);
    //   az_span productName = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT2);
    //   result = generate_properties_update_response(azure_iot, productName, &product, version, buffer, buffer_size, response_length);
    //   EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "generate_properties_update_response failed.");

    // }
    // if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT3)))
    // {
    //   azrc = az_json_reader_next_token(&jr);
    //   EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties next token.");
    //   char product;
    //   azrc = az_json_token_get_string(&jr.token, &product, 9999, NULL);
    //   az_span productSpan = az_span_create_from_str(&product);
    //   az_span productName = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT3);
    //   result = generate_properties_update_response(azure_iot, productName, &product, version, buffer, buffer_size, response_length);
    //   EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "generate_properties_update_response failed.");

    // }
    // if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT4)))
    // {
    //   azrc = az_json_reader_next_token(&jr);
    //   EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties next token.");
    //   char product;
    //   azrc = az_json_token_get_string(&jr.token, &product, 9999, NULL);
    //   az_span productSpan = az_span_create_from_str(&product);
    //   az_span productName = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT4);
    //   result = generate_properties_update_response(azure_iot, productName, &product, version, buffer, buffer_size, response_length);
    //   EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "generate_properties_update_response failed.");
...
int azure_pnp_send_telemetry(azure_iot_t* azure_iot,  char* event, char* product, int enteredAmount, int dispensedAmount, int price, char* date, float product1Mass, float product2Mass, float product3Mass, float product4Mass)
{
           ...
           difftime(now, last_telemetry_send_time) >= telemetry_frequency_in_seconds)
  {
    ...
    const char* stockTake = "Stock Take";
    const char* startChar = "Start";
    const char* fin ="Finished";
    const char* amount_chosen="Amount Chosen";

    if (strcmp(event, stockTake)==0){
        LogInfo("STOCK TAKE");
            if (generate_telemetry_payload(data_buffer, DATA_BUFFER_SIZE, &payload_size,event,product1Mass,product2Mass,product3Mass,product4Mass) != RESULT_OK)
    {
      LogError("Failed generating stock take telemetry payload.");
      return RESULT_ERROR;
    }
    if (azure_iot_send_telemetry(azure_iot, az_span_create(data_buffer, payload_size)) != 0)
    {
     ...
     if (generate_telemetry_dispense_payload(data_buffer, DATA_BUFFER_SIZE, &payload_size, event, product, enteredAmount, dispensedAmount, price, date) != RESULT_OK)
    {
      LogError("Failed generating stock take telemetry payload.");
      return RESULT_ERROR;
    }
    ...
    // else if (strcmp(event, startChar)==0){
    // LogInfo("Start");
    //     // generate_telemetry_start_dispense_payload(uint8_t* payload_buffer, size_t payload_buffer_size, size_t* payload_buffer_length, char* event,char* product)
    // if (generate_telemetry_start_dispense_payload(data_buffer, DATA_BUFFER_SIZE, &payload_size, event, product) != RESULT_OK)
    // {
    //   LogError("Failed generating start telemetry payload.");
    //   return RESULT_ERROR;
    // }
    // if (azure_iot_send_telemetry(azure_iot, az_span_create(data_buffer, payload_size)) != 0)
    // {
    ...
    // if (strcmp(event, fin)==0){
    //     LogInfo("Finished");
    // if (generate_telemetry_dispense_payload(data_buffer, DATA_BUFFER_SIZE, &payload_size, event, product, enteredAmount, dispensedAmount) != RESULT_OK)
    // {
    //   LogError("Failed generating stock take telemetry payload.");
    //   return RESULT_ERROR;
    // }
