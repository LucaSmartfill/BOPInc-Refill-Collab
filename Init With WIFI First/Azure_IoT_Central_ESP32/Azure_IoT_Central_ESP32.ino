 ...
 * the MQTT client of your choice to perform all the steps needed to connect and interact with P
 * Azure IoT Central.
 * 
...
// Serial Connection to other board
#include <HardwareSerial.h>
HardwareSerial ESPSIM(1);
...
char* Product1 = "{\"name\":\"Sunsilk\",\"pricePerSachet\":3.4, \"timePerSachet\":1}";
char* Product2 = "{\"name\":\"Sunsilk\",\"pricePerSachet\":3.6, \"timePerSachet\":2}";
char* Product3 = "{\"name\":\"Sunsilk\",\"pricePerSachet\":3.7, \"timePerSachet\":3}";
char* Product4 = "{\"name\":\"Sunsilk\",\"pricePerSachet\":3.5, \"timePerSachet\":4}";
...
static az_span choiceSpan = AZ_SPAN_FROM_STR("Sunsilk");
...
static int mqtt_client_publish_function(mqtt_client_handle_t mqtt_client_handle, mqtt_message_t* mqtt_message)
{
  ...
  LogInfo("MQTT client PAYLOAD is  '%.*s'", az_span_size(mqtt_message->payload), az_span_ptr(mqtt_message->payload)); 


  // char *topic = "$iothub/twin/GET/?$rid="+;

  char topic[25];
  snprintf(topic,sizeof(topic),"$iothub/twin/GET/?$rid=%i", properties_request_id++);

  SerialMon.println(topic);
  az_span propTopic = az_span_create_from_str(topic);
  az_span propPayload = AZ_SPAN_LITERAL_FROM_STR("");


  int propertyRes = esp_mqtt_client_publish((esp_mqtt_client_handle_t)mqtt_client_handle, 
    (const char*)az_span_ptr(propTopic), // topic is always null-terminated.
    (const char*)az_span_ptr(propPayload), 
    az_span_size(propPayload),
    (int)mqtt_message->qos, 
    MQTT_DO_NOT_RETAIN_MSG);


  SerialMon.println("PROPERTY GET REQUEST");
...
{
  LogInfo("Properties received: %.*s", az_span_size(properties), az_span_ptr(properties));
...
void get_property(azure_iot_t* azure_iot){
  int result;
  az_result azrc;
  az_json_reader jr;
  az_span component_name ;
  int32_t version = 0;
  SerialMon.println("Get Prop Function");
  ...
  while (az_result_succeeded(
    azrc = az_iot_hub_client_properties_get_next_component_property(
      ...
      AZ_IOT_HUB_CLIENT_PROPERTY_REPORTED_FROM_DEVICE, &component_name)))
  {

    SerialMon.println("Get Prop Function2");
    if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR("Product1")))
    {
      ...
      az_span productName = AZ_SPAN_FROM_STR("Product1");
      LogInfo("Product 1: %s", product);
...
void setup()
{
  ...
  ESPSIM.begin(115200, SERIAL_8N1, 19, 18);
  ...
  else
  {
    switch(azure_iot_get_status(&azure_iot))
    {
      case azure_iot_connected:
        ...
        {
            azure_pnp_send_telemetry(&azure_iot,choiceSpan, 10 , 10*10); 
        }
