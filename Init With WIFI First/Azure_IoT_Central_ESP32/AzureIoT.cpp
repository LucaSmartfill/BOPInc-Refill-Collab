...
int handle_prop_get_request(azure_iot_t* azure_iot, az_span properties){
LogInfo("%.*s", az_span_size(properties), az_span_ptr(properties));
char property;

// az_span_to_str(&property, az_span_size(properties)+1, properties);
// // az_result azrc;

// LogInfo("Handeling Resposnse");
// // LogInfo("%s", property);
// LogInfo("Handeling Resposnse3");
return RESULT_OK;
    ...
    if (az_result_succeeded(azrc))
    {
      ...
      switch (property_message.message_type)
      {
        ...
        case AZ_IOT_HUB_CLIENT_PROPERTIES_MESSAGE_TYPE_GET_RESPONSE:
          // TODO: change to RESULT_OK once implemented.
          
            LogInfo("PROPERTY GET REQUEST");
            handle_prop_get_request(azure_iot, mqtt_message->payload); 
            result = RESULT_OK; 
