...
#define AZURE_PNP_MODEL_ID "dtmi:smartfillDev:BoPIncDispenser_6yy;1"
...
#define TELEMETRY_PROP_NAME_Dispenser1Mass  "Dispenser1Mass"
#define TELEMETRY_PROP_NAME_Dispenser2Mass  "Dispenser2Mass"
#define TELEMETRY_PROP_NAME_Dispenser3Mass  "Dispenser3Mass"
#define TELEMETRY_PROP_NAME_Dispenser4Mass  "Dispenser4Mass"
...
#define WRITABLE_PROPERTY_PRODUCT1         "Product1"
...
static size_t telemetry_frequency_in_seconds = 300; // With default frequency of once in 5 minute 0 seconds.
...
static int generate_telemetry_payload(
  uint8_t* payload_buffer, size_t payload_buffer_size, size_t* payload_buffer_length,az_span product,  float volume, float price);
...
int azure_pnp_send_telemetry(azure_iot_t* azure_iot,az_span product,  float volume, float price)
{
           ...
           difftime(now, last_telemetry_send_time) >= telemetry_frequency_in_seconds)
  {
    ...
    if (generate_telemetry_payload(data_buffer, DATA_BUFFER_SIZE, &payload_size, product,volume, price) != RESULT_OK)
    {
...
int get_properties(azure_iot_t* azure_iot, uint32_t request_id){
  LogInfo("GETTING PROPETRRY REQUEST");
  char properties;
    // az_span rId = az_span_i32toa(request_id)
  ...
  request_id_span = az_span_slice(request_id_span, 0, az_span_size(request_id_span));
  uint32_t len = 999;
  int result = az_iot_hub_client_properties_document_get_publish_topic(&azure_iot->iot_hub_client, request_id_span,&properties, 9999, &len);
  EXIT_IF_TRUE(result != RESULT_OK, RESULT_ERROR, "Failed getting reported properties");
  return RESULT_OK;
}
...
int azure_pnp_handle_properties_update(azure_iot_t* azure_iot, az_span properties, uint32_t request_id)
{
  ...
  int result;
  size_t length;
  LogInfo("Handle prop request ---- %s","OVER HERE");
...
// Will implement reading from load cells here this is just a placeholder for now
static double readDispenser1(){
  return random()*1580;
}
static double readDispenser2(){
  return random()*1600;
}
static double readDispenser3(){
  return random()*1800;
}
static double readDispenser4(){
  return random()*1400;
}
...
static int generate_telemetry_payload(uint8_t* payload_buffer, size_t payload_buffer_size, size_t* payload_buffer_length, az_span product,float volume, float price)
{
  az_json_writer jw;
  az_result rc;
  ...
  float Dispenser1Mass = readDispenser1();
  float Dispenser2Mass = readDispenser2();
  float Dispenser3Mass = readDispenser3();
  float Dispenser4Mass = readDispenser4();
  ...
  uint8_t* buffer, size_t buffer_size, size_t* response_length)
{
      ...
      AZ_IOT_HUB_CLIENT_PROPERTY_WRITABLE, &component_name)))
  {
    ...
    if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT1)))
    {
      ...
      az_span productName = AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_PRODUCT1);
